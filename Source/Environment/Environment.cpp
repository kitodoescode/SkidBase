...
std::string drawing = R"(local coreGui = game:GetService("CoreGui")
...
std::vector<const char*> blockedFuncs =
{
...
int nameCallHook(lua_State* L) {
	...
	if (L->userdata->Identity >= 8 && L->userdata->Script.expired()) {
		...
		if (!data || !RBX::CheckMemory((uintptr_t)data) || !*data) return 0;

		if (strcmp(data, "HttpGet") == 0 || strcmp(data, "HttpGetAsync") == 0) {
			return Library::Http::httpget(L);
		}

		if (strcmp(data, "GetObjects") == 0 || strcmp(data, "GetObjectsAsync") == 0) {
			return Library::Misc::getobjects(L);
		}

		for (const std::string& func : blockedFuncs) {
...
void CEnvironment::Initialize(lua_State* L) {
    ...
    RBX::Print(1, "Misc Library");
    Library::Misc::RegisterLibrary(L);

    RBX::Print(1, "Script Library");
    Library::Script::RegisterLibrary(L);

    RBX::Print(1, "Instances Library [SKIPPED]");
    //Library::Instances::RegisterLibrary(L);

    RBX::Print(1, "Cache Library");
    Library::Cache::RegisterLibrary(L);

    RBX::Print(1, "Closure Library");
    Library::ClosureLib::RegisterLibrary(L);

	RBX::Print(1, "Http Library");
	Library::Http::RegisterLibrary(L);

	RBX::Print(1, "Debug Library [SKIPPED]");
	//Library::Debug::RegisterLibrary(L);

	RBX::Print(1, "MetaTable Library");
	Library::MetaTable::RegisterLibrary(L);

	RBX::Print(1, "Drawing Library");
	Execution->Execute(L, drawing);

	RBX::Print(1, "Init Hooks.");
	InitializeHooks(L);

    RBX::Print(1, "Environment Loaded.");
};