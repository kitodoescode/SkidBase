...
namespace sha3_256 {
    ...
    constexpr size_t sha3_256_DIGEST_LEN = sha3_256::DIGEST_LEN;
    ...
    std::string sha3_256_hash(std::string Message) {
        ...
        std::vector<uint8_t> dig(sha3_256_DIGEST_LEN, 0);
        auto _dig = std::span<uint8_t, sha3_256_DIGEST_LEN>(dig);
