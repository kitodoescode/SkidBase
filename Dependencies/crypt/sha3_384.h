...
namespace sha3_384 {
    ...
    constexpr size_t sha3_384_DIGEST_LEN = sha3_384::DIGEST_LEN;

    // Given arbitrary many input message bytes, this routine consumes it into keccak[768] sponge state and squeezes out 48
    // -bytes digest.
    //
    ...
    inline std::string to_hex_from_bytes(std::span<const uint8_t> bytes)
    {
        std::stringstream ss;
        ss << std::hex;
    ...
    inline std::string sha3_384_hash(std::string Message) {
        ...
        std::vector<uint8_t> dig(sha3_384_DIGEST_LEN, 0);
        auto _dig = std::span<uint8_t, sha3_384_DIGEST_LEN>(dig);
