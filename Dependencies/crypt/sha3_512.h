...
namespace sha3_512 {
    ...
    constexpr size_t sha3_512_DIGEST_LEN = sha3_512::DIGEST_LEN;
    ...
    public:
        // Constructor
        forceinline constexpr sha3_512_t() = default;
    ...
    std::string sha3_512_hash(std::string Message) {
        ...
        std::vector<uint8_t> dig(sha3_512_DIGEST_LEN, 0);
        auto _dig = std::span<uint8_t, sha3_512_DIGEST_LEN>(dig);
